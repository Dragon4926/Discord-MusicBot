...
module.exports = (client, inter) => {
    ...
    if (command.permissions && !inter.member.permissions.has(command.permissions)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ |Permission Denied| \nYou need do not have the proper permissions to exacute this command`)], ephemeral: true, })
    if (DJ.enabled && DJ.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | Invalid Access | Access limited to \`${DJ.roleName}\` `)], ephemeral: true, })
